...
#define TAG "com.dcgov.parsedc"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Parsedc_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Parsedc.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ParsedcBindings::lookupGeneratedInit(
...
static void Parsedc_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Parsedc_getBinding)->GetFunction());
}

static void Parsedc_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ParsedcBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ParsedcBinding = {
	"com.dcgov.parsedc",
	Parsedc_init,
	Parsedc_dispose
};
...
Java_com_dcgov_parsedc_ParsedcBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.dcgov.parsedc", &ParsedcBinding);
	titanium::KrollBindings::addExternalLookup(&(::ParsedcBindings::lookupGeneratedInit));
}
